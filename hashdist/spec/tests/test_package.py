...
def test_parse_deps_repeated():
    ...
    with assert_raises(PackageError) as e:
        package.parse_deps_and_constraints(loads("{'dependencies': {'build': ['a', '+a']}}"))
    assert 'dependency a repeated' in str(e.exc_val)
    with assert_raises(PackageError):
        package.parse_deps_and_constraints(loads("{'dependencies': {'run': ['a', '+a']}}"))
    assert 'dependency a repeated' in str(e.exc_val)
    # But this is OK:
    package.parse_deps_and_constraints(loads("{'dependencies': {'run': ['a'], 'build': ['a']}}"))
